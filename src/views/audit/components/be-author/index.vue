      ...
      <div class="search-form">
        <el-form :inline="true" :model="authorSearchForm" class="demo-form-inline" size="small">
          ...
          <el-form-item>
            ...
            <el-button type="primary" icon="el-icon-delete" size="small" @click="authorSearchForm.roleName = ''">重置</el-button>
          </el-form-item>
        </el-form>
      </div>
      <div class="table-represent">
        <el-table
          ...
          <el-table-column
            ...
            width="220">
            <template slot-scope="scope">
              <el-button v-if="noAuthShowBtn || power['审核通过']" type="text" @click="pass(scope.row)">通过</el-button>
              <el-button v-if="noAuthShowBtn || power['审核不通过']" type="text" @click="noPass(scope.row)">不通过</el-button>
            </template>
          </el-table-column>
        </el-table>
...
<script>
  ...
  import { authorAuditList, authorAuditPass, authorAuditNoPass } from '../../../../api/audit'
  ...
  export default {
    ...
    methods: {
      ...
      pass(item) {
        authorAuditPass({ auditId: item.auditId }).then(res => {
          if (res.flag) {
            this.$message({
              message: '提交成功',
              type: 'success'
      ...
      noPass(item) {
        this.$prompt('审核不通过理由', '提示', {
          ...
          authorAuditNoPass({ auditId: item.auditId, reason: value }).then(res => {
            if (res.flag) {
              this.$message({
                message: '提交成功',
                type: 'success'
